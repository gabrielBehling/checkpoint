...
router.delete("/teams/:teamId/members/:memberId", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (teamCaptain !== userId && memberId !== userId) {
      return res.error("You are not authorized to remove this member", "UNAUTHORIZED", 403, { teamId, memberId, userId });
    }
...
router.post("/:eventId/teams", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (existingTeamResult.recordset.length > 0) {
      return res.error("You have already created a team for this event", "TEAM_ALREADY_EXISTS", 400, { eventId, userId });
    }
    ...
    if (maxTeams !== null && currentTeamCount >= maxTeams) {
      return res.error("Maximum number of teams reached for this event", "MAX_TEAMS_REACHED", 400, {
        eventId,
        currentTeamCount,
        maxTeams,
      });
...
router.get("/teams/:teamId", async (req, res) => {
  ...
  try {
    ...
    const teamResultAndMemberResult = await sql.query`
      SELECT 
          T.*,
          ER.EventId,
          ER.Status,
  ...
  `;
    if (teamResultAndMemberResult.recordsets[0].length === 0) {
      return res.error("Team not found", "TEAM_NOT_FOUND", 404);
    }

    const teamData = teamResultAndMemberResult.recordsets[0][0];


    const members = teamResultAndMemberResult.recordsets[1].map((member) => ({
...
router.post("/teams/:teamId/join", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (maxMembers !== null && currentMemberCount >= maxMembers) {
      return res.error("Team is already full", "TEAM_FULL", 400, {
        teamId,
    ...
    if (memberResult.recordset.length > 0) {
      return res.error("You are already a member of this team", "ALREADY_MEMBER", 400, { teamId, userId });
    }
...
router.delete("/teams/:teamId", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (teamResult.recordset[0].CreatedBy !== userId) {
      return res.error("You are not authorized to delete this team", "UNAUTHORIZED", 403, { teamId, userId });
    }
    ...
    return res.success({ teamId }, "Team deleted successfully");
  } catch (error) {
    return res.error(error.message, "INTERNAL_ERROR", 500);
  } finally {
    if (connection) {
