...
```
https://checkpoint.localhost/api/auth
```
...
```bash
curl -X PUT https://checkpoint.localhost/api/auth/update-info \
...
```bash
curl -X POST https://checkpoint.localhost/api/auth/login \
...
```bash
curl -X POST https://checkpoint.localhost/api/auth/register \
  -H "Content-Type: application/json" \
  -d '{
    "username": "novousuario",
    "email": "novo@email.com",
...
```bash
curl -X POST https://checkpoint.localhost/api/auth/refresh-token \
...
```bash
curl https://checkpoint.localhost/api/auth/me \
  -H "Cookie: accessToken=your_token"
```
...
```bash
curl -X POST https://checkpoint.localhost/api/auth/request-password-reset \
  -H "Content-Type: application/json" \
  -d '{
    "email": "usuario@email.com"
  }'
...
```bash
curl -X POST https://checkpoint.localhost/api/auth/reset-password \
  -H "Content-Type: application/json" \
  -d '{
    "token": "token_recebido_no_email",
...
```bash
curl -X POST https://checkpoint.localhost/api/auth/logout \
