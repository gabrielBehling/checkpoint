...
```
https://checkpoint.localhost/api/chat
```
...
**URL:** `https://checkpoint.localhost`  
**Path:** `/api/chat/socket.io`

**Configuração do Cliente:**

```javascript
const socket = io("https://checkpoint.localhost", {
  path: "/api/chat/socket.io",
...
```
https://checkpoint.localhost/api/chat/uploads/[filename]
```

**Exemplo:**

```
https://checkpoint.localhost/api/chat/uploads/1704115200000-screenshot.png
```
...
fetch(`https://checkpoint.localhost/api/chat/messages/${teamId}`, {
  credentials: "include",
})
  .then((res) => {
    if (!res.ok) {
...
fileInput.addEventListener("change", async (event) => {
  ...
  try {
    const response = await fetch("https://checkpoint.localhost/api/chat/upload", {
      method: "POST",
      credentials: "include",
      body: formData,
    });
...
fetch(`https://checkpoint.localhost/api/chat/events/${eventId}/comments`, {
  method: "POST",
  credentials: "include",
  headers: {
    "Content-Type": "application/json",
...
function displayMessage(message) {
  ...
  if (message.type === "text") {
    ...
    content += `<div class="message-image">
      <img src="https://checkpoint.localhost/api/chat${message.fileUrl}" alt="Imagem enviada">
    </div>`;
  } else if (message.type === "audio") {
    if (message.message) {
      content += `<div class="message-text">${safeText}</div>`;
    ...
    content += `<div class="message-audio">
      <audio controls src="https://checkpoint.localhost/api/chat${message.fileUrl}"></audio>
    </div>`;
  }
