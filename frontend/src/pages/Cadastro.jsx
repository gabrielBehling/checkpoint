      ...
      <main className="container">
        <section className="form-auth form-cadastro">
          ...
          <form onSubmit={handleSubmit}>
            ...
            <input type="email" id="email-cadastro" placeholder="Insira seu endereço de e-mail..." required value={email} onChange={(e) => setEmail(e.target.value)} />
            {emailError && <div className="error-message">{emailError}</div>}

            <label htmlFor="nome-usuario">Nome de Usuário</label>
            <input type="text" id="nome-usuario" placeholder="Insira seu nome de usuário..." required value={nome} onChange={(e) => setNome(e.target.value)} />
            {usernameError && <div className="error-message">{usernameError}</div>}

            <label htmlFor="profile-file">Foto de Perfil (opcional)</label>
            <input type="file" id="profile-file" accept="image/*" onChange={(e) => setProfileFile(e.target.files?.[0] || null)} />
            ...
            <div className="password-input-wrapper1">
              <input type={showPassword ? "text" : "password"} id="senha-cadastro" placeholder="Insira sua senha..." required value={password} onChange={(e) => setPassword(e.target.value)} name="password" pattern="^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d@$!%*?&#\.]{8,}$" title="A senha deve conter pelo menos uma letra, um número e ter no mínimo 8 caracteres." />

              {/* OLHO MÁGICO (igual login) */}
              <button type="button" className="password-toggle-btn1" onClick={togglePasswordVisibility} aria-label="Alternar visibilidade da senha">
                {showPassword ? (
                  /* Olho aberto */
                  <svg xmlns="https://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            ...
            <div className="password-input-wrapper1">
              <input type={showConfirmPassword ? "text" : "password"} id="confirmar-senha" placeholder="Insira a senha novamente..." required value={passwordConfirm} onChange={(e) => setPasswordConfirm(e.target.value)} />

              {/* OLHO MÁGICO 2 */}
              <button type="button" className="password-toggle-btn2" onClick={toggleConfirmPasswordVisibility} aria-label="Alternar visibilidade da confirmação de senha">
                {showConfirmPassword ? (
                  <svg xmlns="https://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                    <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path>
                    <circle cx="12" cy="12" r="3"></circle>
