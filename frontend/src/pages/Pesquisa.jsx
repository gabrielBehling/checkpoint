...
function SearchPage() {
  ...
  useEffect(() => {
    const fetchFilterOptions = async () => {
      try {
        ...
        const response = await fetch(`https://checkpoint.localhost/api/events/filters`);
        if (!response.ok) {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      ...
      try {
        ...
        const response = await fetch(`https://checkpoint.localhost/api/events?${params.toString()}`);
  ...
  const renderPagination = () => {
    ...
    if (startPage > 1) {
      pageButtons.push(
        <button key="start-ellipsis" className="pagination-btn" onClick={() => goToPage(startPage - 1)}>
          ...
        </button>
      );
    }
    ...
    for (let i = startPage; i <= endPage; i++) {
      pageButtons.push(
        <button key={i} className={`pagination-btn ${i === currentPage ? "active" : ""}`} onClick={() => goToPage(i)} disabled={i === currentPage}>
          {i}
        </button>
      );
    }
    ...
    if (endPage < lastPage) {
      pageButtons.push(
        <button key="end-ellipsis" className="pagination-btn" onClick={() => goToPage(endPage + 1)}>
          ...
        </button>
      );
    }
    ...
    return (
      <div className="pagination-container">
        ...
        <button className="pagination-btn" onClick={() => goToPage(currentPage + 1)} disabled={currentPage === lastPage}>
          Próx.
        </button>
        <button className="pagination-btn" onClick={() => goToPage(lastPage)} disabled={currentPage === lastPage}>
          Últ.
        </button>

        <form onSubmit={handlePageInputSubmit} className="pagination-form">
          <input type="number" className="pagination-input" value={pageInput} onChange={(e) => setPageInput(e.target.value)} min="1" max={lastPage} />
    ...
    <div>
      ...
      <div className="search-page-container">
        ...
        <div className="search-bar-wrapper">
          <input type="text" className="search-input" placeholder="Buscar eventos..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} onKeyPress={(e) => e.key === "Enter" && handleSearch()} />
              ...
              {filtersLoading ? (
                ...
                <div className="filters-grid">
                  ...
                  <div className="checkbox-wrapper">
                    <input id="isOnline" name="isOnline" type="checkbox" checked={filters.isOnline} onChange={handleChange} />
                    <label htmlFor="isOnline">Apenas eventos online</label>
                  </div>
                </div>
              )}
        ...
        <div>
          {loading ? (
            ...
            <div className="empty-state">Nenhum evento encontrado com os filtros aplicados.</div>
          ) : (
            <>
                  ...
                  <div key={event.eventId} className="event-card">
                    ...
                    <div className="event-card-content">
                      <div>
                        ...
                        <div className="event-details-grid">
                          ...
                          <div className="event-detail-item">
                            ...
                            <span> {filterOptions.modes.find((m) => m.ModeName === event.mode)?.ModeName || event.mode}</span>
                          </div>
                          <div className="event-detail-item">
                            <strong>Local:</strong>
                      ...
                      <div className="event-footer">
                        <div className="event-price">{event.ticket ? `R$ ${event.ticket.toFixed(2)}` : "Grátis"}</div>
